...
function generateCustomerComment(productName: string, customerType: CustomerType, formula: string): string {
  ...
  const usageComments: Record<string, string[]> = {
    // Level 1 products (通常数字版のみ)
    'H2O': [
      'のどがかわいた！飲み水として水を作ってくれ！',
      '植物への水やりに純粋な水が必要だ！',
      '料理に使う清潔な水がほしいんだ！',
      '化学実験用の蒸留水を作ってくれ！'
    ],
    'MgO': [
      '胃薬として制酸剤に酸化マグネシウムが必要だ！',
      '製鉄所の耐火れんが製造に使いたい！',
      '便秘薬の原料として酸化マグネシウムを！',
      'セメント工場で特殊セメント作りに使うよ！'
    ],
    'Al2O3': [
      'アルミニウム製錬の原料として酸化アルミニウムが必要！',
      'サンドペーパーの研磨剤に使いたいんだ！',
      '人工ルビー製造に高純度酸化アルミニウムを！',
      'セラミックス工場で耐熱材料として使うよ！'
    ],
    'NH3': [
      '農業用窒素肥料の原料としてアンモニアが必要だ！',
      '冷凍システムの冷媒としてアンモニアを使いたい！',
      '洗剤製造でアンモニア水を作るんだ！',
      'ハーバー法で作った高純度アンモニアがほしい！'
    ],
    'CO2': [
      '炭酸飲料製造に食品グレードの二酸化炭素を！',
      '消火器の不燃性ガスとして使いたい！',
      'ドライアイス製造に液体二酸化炭素が必要だ！',
      '植物工場で光合成促進用のCO2を！'
    ],
    'NaCl': [
      '料理の基本調味料として食塩を作ってくれ！',
  ...
  const baseComment = usageComments[normalizedFormula] 
    ...
    : `${productName}が必要なんだ。用途は秘密だよ。`;

  // 客タイプに応じて表現を調整
  switch (customerType) {
    case 'rare':
      return baseComment.replace(/！$/, '！大量に頼む！');
    case 'super rare':
      return baseComment.replace(/！$/, '！工場レベルで大量生産してくれ！');
    case 'vip':
      return baseComment.replace(/。$/, '。最高品質でお願いします。').replace(/！$/, '。VIP仕様で特別に作っていただけますか？');
    default:
      return baseComment;
  }
}
